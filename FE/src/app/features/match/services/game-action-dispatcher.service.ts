...
export class GameActionDispatcherService {
  ...
  private readonly actionRules: Record<string, ActionRule> = {
    ...
    CHOOSE_KNOCKOUT_REPLACEMENT:   { allowedStatuses: ['ACTIVE'], needsMyTurn: false, requiresCondition: 'PENDING_KO_REPLACEMENT' },
  };

  private readonly conditionCheckers: Record<string, () => boolean> = {
    PENDING_PRIZE: () => {
      console.warn('[GameActionDispatcher] Condition PENDING_PRIZE not yet implemented — blocking action');
      return false;
    },
    PENDING_KO_REPLACEMENT: () => {
      console.warn('[GameActionDispatcher] Condition PENDING_KO_REPLACEMENT not yet implemented — blocking action');
      return false;
    },
  };
  ...
  dispatchAction(
    ...
    if (this.matchState.connectionStatus() !== 'DISCONNECTED') {
      this.matchSocket.sendAction(request);
